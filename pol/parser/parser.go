...
func printError(tok *common.Token, msg string) {
	...
	//TODO take second pass at errsystem
	err:= fmt.Sprintf("Symantic Error: %s: Line %d: Token [%s]", msg, tok.Lineno, tok.Value)

	if err== prevError {
	    fmt.Println("Infinite Parsing [PAUL YOU SHOULD REALLY FIX THIS]")
	}

    common.AppendError(err)
...
func Parse(tokens *Tokens) *common.Tree {
	...
	for next, hasNext := tokens.peek(); hasNext; next, hasNext = tokens.peek() {
		node := compilation_unit(tokens)
		if !node.IsInvalidTree() {
			tree.Append(node)
		}
		changed, _ := tokens.peek()
...
func compilation_unit(tokens *Tokens) (*common.Tree) {
...
func single_body(tokens *Tokens) *common.Tree {
	...
	if first.Type == common.IDENTIFIER && second.Type == common.LEFT_PAREN {
		...
		printError(first,"Unexpected item in body")
	}
...
func listify_array(tokens *Tokens, verbose bool) *common.Tree {
	...
	node := common.MakeTree(&common.Token{common.TREE_TEMP, "TEMP", -1})
	node = node.Append(common.MakeTree(&common.Token{common.INTEGER_CONST, strconv.Itoa(index), -1}))
	node = node.Append(value)
	index = index + 1
	return node
}
...
func createInitalizationTree(lhs []*common.Tree, rhs []*common.Tree, op *common.Token) *common.Tree {
	...
	for i, next := range lhs {
		single_asgn := common.MakeTree(&common.Token{common.ASSIGNMENT_OP,"=",op.Lineno})
		single_asgn = single_asgn.Append(next)
		rValue := common.MakeInvalidTree()
		if i < len(rhs) {
			rValue = rhs[i]
		} else {
			rValue = common.MakeTree(&common.Token{common.ZERO_VALUE, "EMPTY", -1})
		}
		if op.Value != "=" {
			rValue = expandOtherEquals(next,op,rValue)
		}
		single_asgn = single_asgn.Append(rValue)
...
func expandOtherEquals(lhs *common.Tree, op *common.Token, rhs *common.Tree) *common.Tree{
	op = getOp(op)
	node := common.MakeTree(op)
	node = node.Append(lhs)
	node = node.Append(rhs)
	return node
}

func getOp(op *common.Token) *common.Token{
	if op.Value == "+=" {
		return &common.Token{common.ADDITIVE_OP,"+",op.Lineno}
	} else if op.Value == "-=" {
		return &common.Token{common.ADDITIVE_OP,"-",op.Lineno}
	} else if op.Value == "*=" {
		return &common.Token{common.MULTIPLICATIVE_OP,"*",op.Lineno}
	} else if op.Value == "/=" {
		return &common.Token{common.MULTIPLICATIVE_OP,"/",op.Lineno}
	} else if op.Value == "%=" {
		return &common.Token{common.MULTIPLICATIVE_OP,"%",op.Lineno}
	}else{
		return &common.Token{common.ILLEGAL,op.Value,op.Lineno}
	}
}
...
func listify_identifier(tokens *Tokens, verbose bool) *common.Tree {
	...
	if !hasNext || next.Type != common.IDENTIFIER {
		...
		_, _ = tokens.next()
		return common.MakeTree(&common.Token{common.VARIABLE,next.Value,next.Lineno})
	}
}
...
func getIdentifier(tokens *Tokens) *common.Tree {
	...
	if token.Type == common.IDENTIFIER {
		id, _ := tokens.next()
		node := common.MakeTree(&common.Token{common.VARIABLE,id.Value,id.Lineno})
		next, _ := tokens.peek()
...
func getFuncCall(tokens *Tokens) *common.Tree {
	...
	_, _ = tokens.next()
	isRightParen, _ := tokens.peek()

	node := common.MakeTree(&common.Token{common.FUNC_CALL,id.Value,id.Lineno})

	if isRightParen.Type != common.RIGHT_PAREN {
...
func getUnary(tokens *Tokens) *common.Tree {
	...
	token, _ := tokens.peek()
	if token.Value == "-" && token.Type != common.ILLEGAL {
		next, _ := tokens.next()
		//subtraction symbol
		...
		node = node.Append(common.MakeTree(&common.Token{common.INTEGER_CONST,"0",-1}))
		node = node.Append(subNode)


		return node

	}else if token.Value == "!" && token.Type != common.ILLEGAL {
		next, _ := tokens.next()
		node := common.MakeTree(next)
		subNode := atom(tokens)
		node = node.Append(subNode)
